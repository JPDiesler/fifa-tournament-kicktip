...
export default function Flag({ code, sm, lg, className = "" }) {
  const cls = lg ? "h-7 w-11" : sm ? "h-3 w-5" : "h-4 w-6";
  const src = (code && FLAGS[code]) || flagUrl(code);
  if (!src) return <span className={`inline-block ${cls} rounded-sm bg-zinc-700 ${className}`} />;
  return <img src={src} alt="" loading="lazy" className={`${cls} rounded-sm object-cover ring-1 ring-black/30 ${className}`} />;
}
