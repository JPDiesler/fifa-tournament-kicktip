...
export default function ScoreInput({ value, onChange, isDisabled, label }) {
  ...
  return (
    ...
    >
      <NumberField.Group className="h-10 w-11 rounded-md border border-border bg-field transition-colors focus-within:border-accent has-[input:disabled]:opacity-40">
        <NumberField.Input className="h-full w-full bg-transparent text-center text-base font-bold text-foreground outline-none" />
      </NumberField.Group>
    </NumberField>
  );
}
